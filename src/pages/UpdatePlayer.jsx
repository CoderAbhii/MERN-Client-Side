...
import { addPlayerFunction, updatePlayerFunction, viewPlayerFunction } from '../api/apis';
import { useNavigate, useParams } from 'react-router-dom';
import { updatePlayer } from '../context/PayerContext';
...
const UpdatePlayerPage = () => {
  ...
  const { setUpdatePlayerState} = useContext(updatePlayer);
  ...
  const getSinglePlayer = async() => {
    ...
    if(apiResponse.status === 200){
      setInputValues(apiResponse.data.playerData);
    }
  }
  ...
  const handleSubmit = async (event) => {
    ...
    if (townhall_player === "") {
      ...
      const apiResponse = await updatePlayerFunction(id, data);
      console.log(apiResponse);
      if(apiResponse.data.updatedBy === "Leader"){
        toast.success(apiResponse.data.successMessage);
        navigate("/get-all-player");
      }
      else if(apiResponse.data.updatedBy === "Co-leader"){
        toast.success(apiResponse.data.successMessage);
        ...
        <div className="add-player">
          ...
          <form className="row" onSubmit={(event) => handleSubmit(event)}>
            ...
            <div className="col-xl-4 m-auto">
              <button className="btn add-player-btn btn-outline-success mt-2">Update</button>
            </div>
          </form>
        </div>
      </div>
